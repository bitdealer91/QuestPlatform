...
export function PlanetsRail({ getStarsForWeek, openTasks }: { getStarsForWeek: (id: number) => 0|1|2|3; openTasks: (id: number) => void }){
	...
	// Unlock first N planets via env; enforce at least 7 to open week 7 drop
	...
	const unlockedCount = Math.max(7, unlockedCountFromEnv);
	...
	return (
		<div ref={containerRef} className="relative w-full h-full overflow-hidden">
			...
			{PLANETS.map(p => {
				...
				const claimEnabled = (p.id >= 1 && p.id <= 7) && !locked; // enable for weeks 1-7
